...
namespace Firebase.Sample.Storage
{
    ...
    // startup.
    public class uihandler_for_categories_scene : MonoBehaviour
    {


        public static String fac_name = Faculty_Details.faculty_name;
        public static String destination_filename_with_extension = Faculty_Details.destination_filename_with_extension_for_uploading;
        public static String local_file_url = Faculty_Details.local_file_url;
        public static String storage_bucket = "";
        public static string folder_name = Faculty_Details.folder_name;
        ...
        protected static string localFilename = local_file_url;
        ...
        {
            Task task;
            uihandler_for_categories_scene uiHandler;

            // Create an enumerator that waits for the specified task to complete.
            public WaitForTaskCompletion(uihandler_for_categories_scene uiHandler, Task task)
            {
        ...
        void GUIDisplayControls()
        {
            if (UIEnabled)
            {
              ...
              (faculty_name.Length > kMaxLogSize ? faculty_name.Substring(kMaxLogSize) : "");
                ...
                GUILayout.Label("Local File Path with extention:");
                localFilename = GUILayout.TextField(localFilename);

                GUILayout.Label("Storage Location:");
                if (String.IsNullOrEmpty(storageLocation))
                {
                    storageLocation = MyStorageBucket + fac_name + "/" + folder_name + "/" + destination_filename_with_extension;
                }
        ...
        void OnGUI()
        {
            ...
            {
                // Portrait mode
                controlArea = new Rect(10000f, 0.0f, Screen.width, Screen.height * 0.5f);
                logArea = new Rect(10000f, Screen.height * 0.5f, Screen.width, Screen.height * 0.5f);
            }
            else
            {
                // Landscape mode
