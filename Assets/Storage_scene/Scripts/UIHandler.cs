...
namespace Firebase.Sample.Storage {
 ...
 using UnityEngine.SceneManagement;
    ...
    public class UIHandler : MonoBehaviour {
        ...
        public static String fac_name = Global.faculty_name;
        public static String destination_filename_with_extension_for_uploading = Global.destination_filename_with_extension_for_uploading;
        public static string destination_filename_with_extension_for_downloading = Global.destination_filename_with_extension_for_downloading;
        public static String local_file_url = Global.local_file_url;
        public static String storage_bucket = "";
        public static string category_name = Global.category_name;
        ...
        public static string localFilename = local_file_url;
        protected static string MyStorageBucket = "";
    ...
    protected Task previousTask;
        ...
        protected virtual void Start() {
            ...
            //while(upload_bool==true)
            //{
            //    StartCoroutine(UploadFromFile());
            //    upload_bool = false;
            //}
            //storageLocation = MyStorageBucket+fac_name+"/" + destination_filename + ".txt";
      ...
      FirebaseApp.CheckAndFixDependenciesAsync().ContinueWithOnMainThread(task => {
        ...
        if (dependencyStatus == DependencyStatus.Available) {
          ...
          Debug.LogError(
            ...
            //if(Global.upload_bool==true)
            //{
            //    StartCoroutine(UploadFromFile());
            //    Global.upload_bool = false;
            //    //SceneManager.LoadScene(2);
                

            //}
            //if (Global.download_bool==true)
            //{
            //    StartCoroutine(DownloadToFile());
            //    Global.download_bool = false;
            //    //SceneManager.LoadScene(2);


            //}
            //if (Global.delete_bool==true)
            //{
            //    StartCoroutine(Delete());
            //    Global.delete_bool = false;
            //    //SceneManager.LoadScene(2);
    ...
    protected void DisplayUploadComplete(Task<StorageMetadata> task) {
      if (!(task.IsFaulted || task.IsCanceled)) {
        ...
        DebugLog("Press the Download button to download text from Cloud Storage\n");
                Global.done_uploading = true;
                //Global.done_pasting = true;
                if (Global.shud_I_delete_after_viewing_file == true)
                {
                    ...
                    Debug.Log("C:" + "\\" + "FacultyDocs" + "\\" + Global.faculty_name + "\\" + Global.year + "\\" + Global.subject + "\\" + Global.semester + "\\" + Global.section + "\\" + Global.category_name + "\\" + Global.destination_filename_with_extension_for_downloading + " has been deleted ");
                    Global.shud_I_delete_after_viewing_file = false;
                }
                SceneManager.LoadScene(2); //done changes here on 31/01/2020 
      }
    }
    ...
    //    Encoding.UTF8.GetBytes(faculty_name), StringToMetadataChange(fileMetadataChangeString),
    ...
    //    new MemoryStream(System.Text.Encoding.ASCII.GetBytes(faculty_name)),
    ...
    protected IEnumerator UploadFromFile() {
            //*********************************************************************************************************************************************************************************
            var localFilenameUriString = localFilename;
            //*********************************************************************************************************************************************************************************
            var storageReference = GetStorageReference();
    ...
    //            faculty_name = System.Text.Encoding.Default.GetString(task.Result);
    ...
    //          faculty_name += System.Text.Encoding.Default.GetString(buffer, 0, read);
    //    }
    //  },
    ...
    protected IEnumerator DownloadToFile() {
      var storageReference = GetStorageReference();
            ...
            //****************************************************************************
            //string dest_file_name_in_cloud = "";
            //for (int i = (localFilename.Length - 1); i >= 0; i--)
            //{
            //    //file_name_length++;
            //    //dest_file_name_rev = dest_file_name_rev + explorer_path[i];
            //    if (localFilename[i] == '\\')
            //    {
            //        Debug.Log("found backslash");
            //        dest_file_name_in_cloud = localFilename.Substring(i + 1);
            //        break;
            ...
            var localFilenameUriString = localFilename;//PathToPersistentDataPathUriString(localFilename);
            //*********************************************************************************************************************************************************************************
      ...
      if (!(task.IsFaulted || task.IsCanceled)) {
        ...
        DebugLog(String.Format("File Size {0} bytes\n", (new FileInfo(filename)).Length));
                faculty_name = File.ReadAllText(filename);
                Global.done_downloading = true;
      ...
      return GUILayout.Button(buttonText, style) && enabled;
        ...
        void GUIDisplayControls() {
            ...
            if (UIEnabled) {
                ...
                //GUILayout.Label("Text Field to create a new .txt File for upload");
                //if (faculty_name == null)
                //    faculty_name = "Sample text... (type here)";

                // Track changes in the downloaded file.
                if (faculty_name != previousFileContents)
                {
                    previousFileContents = faculty_name;
                    // TextArea can only display a subset of the file contents so limit what is displayed.
                    editableFileContents = faculty_name;
                ...
                //fileContentsScrollViewVector = GUILayout.BeginScrollView(fileContentsScrollViewVector, GUILayout.MinHeight(textAreaLineHeight * 6));
              ...
              //  {
              //      faculty_name = newEditableFileContents +
              //(faculty_name.Length > kMaxLogSize ? faculty_name.Substring(kMaxLogSize) : "");
              //  }
                ...
                GUILayout.Label("Chosen File Path with extention:");
        localFilename = GUILayout.TextField(localFilename);

                GUILayout.Label("Storage Location:");
                if (String.IsNullOrEmpty(storageLocation)) {
                    //**********************************************************************************************************************
          storageLocation = MyStorageBucket + fac_name +"/"+Global.year+"/"+Global.subject + "/" + Global.semester + "/"+ Global.list_of_sections_to_upload_to[Global.current_uploading_file_counter] + "/"+Global.category_name+"/"+ Global.destination_filename_with_extension_for_uploading+Global.destination_filename_with_extension_for_downloading;
        }//*********************************************************************************************************************************
        ...
        GUILayout.BeginVertical();
                ...
                //                File.Copy(Global.local_file_url, "C:/FacultyDocs" + "/" + Global.faculty_name + "/" + Global.year + "/" + Global.subject + "/" + Global.section + "/" + Global.category_name + "/" + categories_management.file_name,true);
                //           //************************uploading to firebase real time database **************************
                //            Firebase.Sample.Database.UIHandler uihandler = new Firebase.Sample.Database.UIHandler();
                //            Global.extension = GetExtension(Global.destination_filename_with_extension_for_uploading);
                //            uihandler.AddScore();
                ...
                if (Button("Exit to Main Screen", !operationInProgress))
                {
                    SceneManager.LoadScene(2);
                }
        ...
        GUILayout.EndScrollView();
                ...
                //****************************************************************************************automatic uploading and downloading done here +************************************
                if (Global.upload_bool == true&& Global.download_bool==false)
                {


                    //**********************uploading to local storage*******************
                    File.Copy(Global.local_file_url, "C:/FacultyDocs" + "/" + Global.faculty_name + "/" + Global.year + "/" + Global.subject + "/" + Global.semester + "/" + Global.section + "/" + Global.category_name + "/" + categories_management.file_name, true);
                ...
                if (Global.download_bool == true && Global.upload_bool==false)
                {
                    StartCoroutine(DownloadToFile());
                    Global.download_bool = false;
                ...
                //if (Global.paste_bool == true)
                //{
                //    StartCoroutine(UploadFromFile());
                //    Global.paste_bool = false;
